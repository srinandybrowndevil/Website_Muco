...
export function LiveFiles({ organizationId, customer = false }: { organizationId: string; customer?: boolean }) {
  ...
  return <section className={customer ? "panel" : "page"}><div className="pagehead"><div><p className="eyebrow">{customer ? "Your deliverables" : "Workspace / Files"}</p><h1>Files</h1><p>{customer ? "Files shared with your customer account." : "Private deliverables, shared only with the selected customer."}</p></div></div>

...
The live customer portal is `portal.mucolabs.com/login`. The domain `portal.megolance.com` appears to be a different or mistyped hostname and must not be treated as an alias until DNS and deployment ownership are confirmed. `danishanachievement.com` is also not currently part of the checked portal flow.

## Confirmed portal boundary

The portal currently uses Supabase Auth for password and Google OAuth, Supabase SSR cookies in `src/proxy.ts`, and Supabase `memberships` plus row-level security to decide whether a user is a customer or staff member. CRM records such as leads, customers, requests and files are protected by that Supabase identity. The existing source already contains both **Continue with Google** on sign-in and **Sign up with Google** on customer registration, but the separate production portal host has not been redeployed in this workspace and still showed the previous Magic Link screen during the last browser check.
